<div>
    @if (auth()->user()->isStudent())
        ...
        <button wire:click="toggleChat"
            ...
            @else
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="h-6 w-6 animate-pulse">
        ...
        @if ($isOpen)
            <div
                class="fixed bottom-24 right-6 z-50 w-96 max-w-[calc(100vw-3rem)] h-[600px] max-h-[calc(100vh-8rem)] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl flex flex-col border border-gray-200 dark:border-gray-700 overflow-hidden backdrop-blur-sm animate-in slide-in-from-bottom-5 duration-300">
                <!-- Header -->
                <div
                    class="bg-gradient-to-r from-purple-600 to-purple-700 dark:from-purple-700 dark:to-purple-800 text-white p-4 flex justify-between items-center shadow-lg">
                    <div class="flex items-center gap-3">
                        <!-- Avatar -->
                        <div
                            class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center ring-2 ring-white/30">
                    ...
                    <button wire:click="toggleChat"
                        class="text-white hover:text-gray-200 hover:bg-white/10 rounded-full p-1.5 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                ...
                <!-- Messages Container -->
                <div class="flex-1 overflow-y-auto p-3 space-y-1.5 bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 chat-scrollbar"
                    id="chat-messages">
                    @foreach ($messages as $index => $message)
                        <div
                            class="flex {{ $message['type'] === 'user' ? 'justify-end' : 'justify-start' }} animate-in fade-in slide-in-from-bottom-2 duration-300">
                            @if ($message['type'] === 'user')
                                <!-- User Message -->
                                <div
                                    class="max-w-[80%] rounded-xl rounded-tr-sm p-2.5 bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-sm">
                                    <div class="text-sm whitespace-pre-wrap leading-snug">{{ $message['content'] }}
                                    </div>
                            ...
                            @else
                                ...
                                <div class="max-w-[85%] flex gap-2">
                                    <!-- Bot Avatar -->
                                    <div
                                        class="w-7 h-7 rounded-full bg-gradient-to-r from-purple-500 to-purple-600 flex items-center justify-center flex-shrink-0 mt-0.5 ring-2 ring-purple-200 dark:ring-purple-900">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="h-3.5 w-3.5 text-white">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                                        </svg>
                                    </div>
                                    <div
                                        class="flex-1 rounded-xl rounded-tl-sm p-2.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm border border-gray-200 dark:border-gray-600">
                                        <div
                                            class="text-sm whitespace-pre-line leading-tight text-gray-800 dark:text-gray-200">
                                            {!! nl2br(e($message['content'])) !!}
                                        </div>
                                        <div class="text-xs mt-1 opacity-70 text-gray-500 dark:text-gray-400">
                                            {{ $message['timestamp']->format('H:i') }}
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        @foreach ($messageButtons[$index] as $button)
                                            ...
                                            <button
                                                ...
                                                <span wire:loading.remove
                                                    wire:target="buttonAction('{{ $action }}', {{ $dataParam }})"
                                                ...
                                                <span wire:loading
                                                    ...
                                                    <x-loading-spinner size="h-3 w-3"
                    ...
                    @if ($isTyping)
                        <div class="flex justify-start animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <div class="flex gap-2 items-end">
                                <!-- Bot Avatar -->
                                <div
                                    class="w-7 h-7 rounded-full bg-gradient-to-r from-purple-500 to-purple-600 flex items-center justify-center flex-shrink-0 ring-2 ring-purple-200 dark:ring-purple-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="h-3.5 w-3.5 text-white">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                                ...
                                <div
                                    class="bg-white dark:bg-gray-700 rounded-xl rounded-tl-sm p-2.5 shadow-sm border border-gray-200 dark:border-gray-600">
                                    <div class="flex space-x-1">
                                        <div class="w-2 h-2 bg-purple-400 dark:bg-purple-500 rounded-full animate-bounce"
                ...
                @if ($currentGame || $showingTipForm)
                    <div
                        class="px-4 py-4 border-t border-gray-200 dark:border-gray-700 bg-gradient-to-t from-gray-50 to-white dark:from-gray-900 dark:to-gray-800/50 backdrop-blur-sm">
                        @if ($currentGame)
                            <div class="flex gap-2">
                                <input type="text" wire:model="gameAnswer"
                                    ...
                                    autocomplete="off">
                                <button wire:click="buttonAction('submit_game_answer', null)"
                                    wire:loading.attr="disabled" wire:target="submit_game_answer"
                        ...
                        @elseif($showingTipForm)
                            <div class="space-y-3">
                                <input type="text" wire:model="tipNickname"
                                    ...
                                    autocomplete="off">
                                <textarea wire:model="tipContent" placeholder="Share your tip or advice here..." rows="3"
                                ...
                                <button wire:click="buttonAction('submit_tip', null)" wire:loading.attr="disabled"
                                    ...
                                    <span wire:loading.remove wire:target="submit_tip"
                                        class="flex items-center justify-center gap-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    ...
                                    <span wire:loading wire:target="submit_tip"
                                        class="flex items-center justify-center gap-2">
                ...
                @else
                    ...
                    <div
                        class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gradient-to-t from-purple-50/50 to-transparent dark:from-purple-900/10 dark:to-transparent">
                        <p
                            class="text-xs text-gray-600 dark:text-gray-400 text-center flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="1.5" stroke="currentColor" class="h-4 w-4 text-purple-500">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                            </svg>
