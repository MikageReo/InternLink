...
class RequestDeferTable extends Component
{
    ...
    private function getFilteredRequests()
    {
        ...
        // Apply sorting
        if ($this->sortField) {
            if (in_array($this->sortField, ['deferID', 'applicationDate', 'committeeStatus', 'coordinatorStatus'])) {
...
<body>
    ...
    <div class="report-info">
        ...
        <p><strong>Note:</strong> Defer requests are for next semester internship period</p>
    </div>';
        ...
        foreach ($requests as $request) {
            ...
            $html .= '<tr>
                <td><strong>' . htmlspecialchars($request->deferID ?? '') . '</strong></td>
                <td>' . htmlspecialchars($request->student->studentID ?? '') . '</td>
                <td>' . htmlspecialchars($request->student->user->name ?? '') . '</td>
                <td class="reason-cell">' . htmlspecialchars(Str::limit($request->reason ?? '', 100)) . '</td>
                <td>' . ($request->applicationDate ? \Carbon\Carbon::parse($request->applicationDate)->format('M d, Y') : '') . '</td>
                ...
                <td>' . htmlspecialchars($request->student->program ?? '') . '</td>
                <td>' . htmlspecialchars($request->student->semester ?? '') . '</td>
                <td>' . htmlspecialchars($request->student->session ?? '') . '</td>
            </tr>';
        }
